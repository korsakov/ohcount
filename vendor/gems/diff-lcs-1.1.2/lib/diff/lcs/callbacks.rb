...
class LCSDiff::Diff::LCS::DiffCallbacks
  ...
  def discard_a(event)
    @hunk << LCSDiff::Diff::LCS::Change.new('-', event.old_position, event.old_element)
  end

  def discard_b(event)
    @hunk << LCSDiff::Diff::LCS::Change.new('+', event.new_position, event.new_element)
  end
...
private
  ...
  #     require 'pp'
  #     pp diffs.map { |e| e.map { |f| f.to_a } }
class LCSDiff::Diff::LCS::ContextDiffCallbacks < LCSDiff::Diff::LCS::DiffCallbacks
  def discard_a(event)
...
class LCSDiff::Diff::LCS::SDiffCallbacks
    # Returns the difference set collected during the diff process.
  attr_reader :diffs
